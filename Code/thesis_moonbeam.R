...
# description: reads in Moonbeam data, expands points, and fits logistic 
# regression models to the data.

# requirements: file path to 'moonbeam_processed.csv'.

# outputs: expanded Moonbeam dataframe, logistic regression models, and
...
moonbeam <- moonbeam %>% mutate("intensity" = 1)
# ggplot(moonbeam, aes(x=x,y=y,color=t))+geom_point()+scale_color_viridis_c()+labs(title="Moonbeam")

current <- list()
for(i in 1:max(moonbeam$t)){
  #convert to data frames
  current[[i]] <- as.data.frame(moonbeam %>% filter(t == i))
  #rename columns
...
ggplot(moonbeam, aes(x=x,y=y,color=t)) + geom_point() + scale_color_viridis_c() + 
  theme_minimal() + labs(x="x", y="y", title="Moonbeam")
ggplot(current[[length(current)]], aes(x=x,y=y,color=t)) + geom_point() 
...
#visualize
ggplot(sample_n(data_final_new %>% filter(intensity==1),40000), aes(x=x,y=y,color=t)) + geom_point()
...
#just using all model from sweetrain summaries
st <- Sys.time()
...
p2 <- ggplot(moonbeam, aes(x = x, y = y, color = t)) + geom_point(alpha=0.3) + 
...
for(i in 1:max(moonbeam$t)){
  #convert to data frames
  current_expanded_new[[i]] <- as.data.frame(moonbeam %>% filter(t == i))
  #rename columns
...
for(j in 1:60){
  major_radius[j] <- major_radius[60]
  minor_radius[j] <- minor_radius[60]
}
#plots
plot(minor_radius)
plot(major_radius)
...
ggplot(sample_n(data_new %>% filter(intensity==1), 40000), aes(x=x,y=y,color=t)) + geom_point() + scale_color_viridis_c() +labs(title="Expanded Moonbeam Sample")
ggplot(sample_n(moonbeam,40000), aes(x=x,y=y,color=t))+geom_point()+scale_color_viridis_c()+labs(title="Moonbeam Sample")
...
ventral_indicator <- ifelse((((data_expanded_final_new$x - 50)^2 / 25^2) + ((data_expanded_final_new$y - 150)^2 / 25^2)) < 1 & data_expanded_final_new$t < 10, 1, 0)
...
ventral_indicator <- ifelse((((data_final_new$x - 50)^2 / 25^2) + ((data_final_new$y - 150)^2 / 25^2)) < 1 & data_final_new$t < 10, 1, 0)
